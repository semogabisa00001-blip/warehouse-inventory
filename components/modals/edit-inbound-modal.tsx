...
export function EditInboundModal({ open, onClose, onSuccess, inboundData }: EditInboundModalProps) {
  ...
  const loadInboundData = async () => {
    setFormData({
      inbound_number: inboundData.inbound_number,
      inbound_date: inboundData.inbound_date,
      supplier: inboundData.supplier
    })

    const items = inboundData.items.map((item: any, index: number) => ({
      id: `${index}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq('inbound_header_id', inboundData.id)
      ...
      const { error: detailsError } = await supabase
        ...
        .insert(
          validItems.map(item => ({
            inbound_header_id: inboundData.id,
            part_id: item.part_id,
            qty: item.qty
          }))
        )
    ...
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent onClose={onClose} className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Edit Inbound Transaction</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label>User</Label>
                  <Input value={inboundData?.inbound_user || username} disabled />
                </div>
              </div>
